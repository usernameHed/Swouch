...
namespace unityEssentials.peek.extensions.editor
{
    ...
    public class EditorChrono
    {
        ...
        public void Reset()
        {
            _isStarted = false;    //cooldown over! reset
            _finalTime = 0;
        }
        ...
        private bool IsReady(bool canDoReset)
        {
            ...
            if ((float)EditorApplication.timeSinceStartup >= _finalTime) //cooldown started, and over!
            {
                if (canDoReset)
                    Reset();
                return (true);
            ...
            return (false); //cooldown started, and running
        }
    }
}